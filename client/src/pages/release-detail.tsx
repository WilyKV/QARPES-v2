...
export default function ReleaseDetail() {
 ...
 <h2 className="text-xl font-semibold text-gray-900">Release non trouvée</h2>
 <p className="text-gray-500 mt-2">Cette release n'existe pas ou a été supprimée.</p>
 <Link href="/releases">
 ...
 <p className="text-gray-500">{typedRelease.name}</p>
 </div>
 </div>
 ...
 <div>
 <p className="text-sm font-medium text-gray-900">Date de recette</p>
 <p className="text-sm text-gray-500">{formatDate(typedRelease.recetteDate)}</p>
 </div>
 </div>
 <div className="flex items-center space-x-3">
 <Calendar className="w-5 h-5 text-blue-500" />
 <div>
 <p className="text-sm font-medium text-gray-900">Date de préprod</p>
 <p className="text-sm text-gray-500">{formatDate(typedRelease.preprodDate)}</p>
 </div>
 </div>
 <div className="flex items-center space-x-3">
 <Calendar className="w-5 h-5 text-green-500" />
 <div>
 <p className="text-sm font-medium text-gray-900">Date de production</p>
 <p className="text-sm text-gray-500">{formatDate(typedRelease.productionDate)}</p>
 </div>
 </div>
 </div>
 </CardContent>
 ...
 <p className="text-xs text-gray-500">{version.description || "Aucune description"}</p>
 </div>
 ...
 <p className="text-gray-500">Aucune version de projet associée à cette release</p>
 </div>
 )}
 </CardContent>
 </Card>
 ...
 <h6 className="font-medium text-gray-900">{versionGitRepo.gitRepo?.name}</h6>
 ...
 <div key={procedure.id} className="flex items-center space-x-2 text-sm p-2 bg-gray-50 rounded">
 {procedure.isCompleted ? (
 <CheckCircle className="w-4 h-4 text-green-500" />
 ) : (
 <Clock className="w-4 h-4 text-orange-500" />
 )}
 <span className={procedure.isCompleted ? "text-green-700" : "text-gray-600"}>
 {procedure.title}
 </span>
 ...
 <p className="text-sm text-gray-500">Aucune procédure</p>
 )}
 </div>
 ))}
 <Separator />
 ...
 <p className="text-gray-500">Aucune procédure trouvée pour cette release</p>
 </div>
 )}
 </CardContent>
 </Card>
